...
def get_geodata(h5name,
    ...
    with h5py.File(h5name, 'r') as f:
        ...
        for ivar in varnames:
            ...
            var_dict[ivar] = (f[topkey]['Geolocation Fields'][ivar]
                              .value.astype(fld_dtype))
        ...
        if 'Profile_time' in var_dict:
            ...
            if proftime2datetime:
                var_dict['Profile_time'] = np.array([start_time + timedelta(seconds=float(i)) for i in var_dict['Profile_time']])  # noqa
...
def read_data(h5name, data_field='Radar_Reflectivity',
    ...
    with h5py.File(h5name, 'r') as f:
        ...
        missval = sw_attr[data_field+'.missing'].value[0][0]
        fillval = sw_attr['_FV_'+data_field].value[0][0]
